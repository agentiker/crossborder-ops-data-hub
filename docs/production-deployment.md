...
- **openclaw-gateway.service**：飞书网关（外部 node 项目），同机调 `127.0.0.1:8000/api/data/*`、`/mcp`；负责告警直投、飞书登录、客户对话。**全部配置见 `docs/openclaw-setup.md`**。
...
> **一键替代**：`bash scripts/setup-server.sh` 幂等完成 4.1–4.6（apt 依赖 / swap / 时区 / linger / MySQL 调优 / uv / cloudflared / nvm+node+openclaw）。下面是它逐步做的事，手动执行或排查时参照。
...
# 4.3 时区——用 Asia/Shanghai(CST)。关键区分（别混，否则排程全偏）：
#   · 操作者在【中国大陆】(看晨报/收告警/对话的人)→ OS 跟操作者走用 CST，让 timer 钟点=中国本地
#     (晨报 07:35=操作者早晨；告警静默 23:00~08:30=别半夜吵中国的人)。
#   · 店铺在【印尼】→ 业务日/GMV 的 WIB(UTC+7) 由 core/timezone 独立写死、与 OS 时区无关，报表不受影响。
#   · "收盘后"任务(unsettled 01:13 / profit 01:33 等)已按 CST 调好落在印尼业务日(WIB 午夜)之后，
#     见各 .timer 注释——所以即便机器物理在印尼，OS 仍用 CST（跟操作者，不跟机房/店铺）。
sudo timedatectl set-timezone Asia/Shanghai
#   仅当【操作者】换到别的时区才改（非店铺/机房所在地）；改则同步各 deploy/systemd/*.timer 的 OnCalendar 设计
...
# 4.6 openclaw（飞书网关，外部 node 项目）——nvm 装 node + npm i -g openclaw + 飞书官方插件。
#     ★ 全部网关配置（channels/agents/bindings/MCP/人格/插件）见 docs/openclaw-setup.md ★
...
# ── 飞书 OAuth 网页免登（/app 登录、bootstrap；独立于 openclaw 的 lark.secrets.json）──
# 按 account_id（租户主键）索引，JSON 串。单租户也用 apps 形式，键须 = DEFAULT_ACCOUNT。
FEISHU_OAUTH__APPS={"ecom-app-gtl":{"app_id":"cli_xxx","app_secret":"yyy"}}
FEISHU_OAUTH__SESSION_SECRET=<secrets#3>
FEISHU_OAUTH__COOKIE_SECURE=true    # 生产 HTTPS 必须 true

# ── 多租户拓扑（单客户独立部署的命门，别省）──
# DEFAULT_ACCOUNT 把 board 冷登录 / token 授权 / MCP 无头查询的回落，全对齐到飞书 agent 的同一
# account（默认 ecom-app 仅适配 hp 存量库）。⚠️ DEFAULT_ACCOUNT、FEISHU_OAUTH__APPS 的键、
# HOST_TO_ACCOUNT 的 value 三处必须一致——不一致则 bootstrap 落错租户、商家店铺挂错 account
# （本项目投产时踩过的 P0）。`uv run python -m scripts.preflight` 会自动校验这三处对齐。
TENANCY__DEFAULT_ACCOUNT=ecom-app-gtl
TENANCY__HOST_TO_ACCOUNT={"board.example.com":"ecom-app-gtl"}
TENANCY__PUBLIC_BASE_URL={"ecom-app-gtl":"https://board.example.com"}

# ── Web 对话 LLM ──
# ⚠️ Anthropic 兼容中转站（如 api.agent0101.com，glm-5.2）也填 anthropic，填 glm 会抛错 500
#    （代码工厂只认 anthropic / openai_compat；BASE_URL 无需加 /v1/messages，AnthropicProvider 自拼）。
LLM__PROVIDER=openai                          # openai 兼容族；中转站 Claude 协议填 anthropic
...
### 5.4 飞书后台 / TikTok Partner Center 登记（外部控制台，仓库外）

`.env` 配好凭据后，还要去两个外部后台登记回调，否则登录/授权会被拒：

**飞书开放平台**（该 account 对应的 app）：
1. 「安全设置 / 重定向 URL」加 `https://<域名>/board/auth/feishu/callback`。**换机/换域名必须加新的**，否则 OAuth 报 `20027`（redirect 不匹配）。
2. 「权限管理」开通 `contact:user.base:readonly`（获取用户基本信息，登录够用）并**发版**，否则 authorize 报 `20027`。

**TikTok Partner Center**：
3. 回调地址登记 `https://<域名>/auth/callback/tiktok`（友好成功页用 `.../tiktok/html`，给商家看）。
4. **出口 IP 白名单**加生产机出口 IP（直连，见 `tiktok-api-direct-connect`）；白名单生效有几分钟延迟，期间 403 正常。

### 5.5 部署前预检

```bash
uv run python -m scripts.preflight        # 必填密钥 / 单租户三处对齐 / DB 连通 / openclaw_bin
```

绿了再 `deploy.sh`。它把「FEISHU_OAUTH__APPS 漏配、租户名实不符」这类启动后才暴露的坑提前查出。
...
### 8.1 收尾三步（审核通过、租户对齐后，本项目实测跑通的链路）

```
①飞书 bootstrap → ②商家 TikTok 授权 → ③手动同步验证 → ④过审后逐个起业务 timer
```

1. **飞书 bootstrap（操作者本人做，自动成 boss）**：浏览器开 `https://<域名>/app`（**是 `/app` 不是 `/board`**——SPA 调 `/api/me` 拿 401 后自动跳飞书 OAuth；`/board` 是飞书 H5 看板）。首登者 `ensure_registration` 自动登记为 boss。验证：`SELECT account_id, role, is_active FROM user_roles;` 应见 `role=boss / is_active=1`，且 `account_id` = `TENANCY__DEFAULT_ACCOUNT`（租户对齐生效）。
2. **商家 TikTok 授权**：客户走授权链接授权店铺。单租户对齐后 token 直落正确 account，**无需** `scripts/shop_admin.py assign`。
   > 坑：客户的 App 内浏览器（战斧 / Ads Manager 等）可能提示「不允许跳转 callback」——**≠ 失败**。TikTok 已把 code 打到我们 callback，查 `platform_tokens` 有该 shop_id（access/refresh 为 `fcr1:` 密文）即成功，勿让客户重授；建议改用 Chrome。
3. **手动同步验证**（业务 timer 还停着，先手动拉一轮确认链路通）：
   ```bash
   ~/.local/bin/uv run python -m flows.sync_inventory     # 出口 IP 须已在 TikTok 白名单
   # 同理 sync_orders / sync_sku_variants / sync_fulfillments / sync_ad_spend
   ```

### 8.2 业务 timer：审核前全停（坑）

提交审核 / 正式运营前，**10 个业务同步 timer 全 disabled，仅 3 个基建 timer（backup/anchor/verify）起**。
> ⚠️ `deploy/deploy.sh` 会 `enable --now` **所有** timer——过审前每次跑完 deploy 都要把业务 timer 手动 `disable --now` 回去（命令见 `docs/ops-runbook.md` §2）。过审后逐个 `enable --now` + `daemon-reload`。
> timer 完整时间表与分类见 `docs/ops-runbook.md` §1。
...
`deploy/deploy.sh` 自动安装 enable `deploy/systemd/` 下全部 `*.service`/`*.timer`。新增 timer 只要丢进该目录重跑 deploy 即生效。常驻服务：`data-hub.service`、`cloudflared-board.service`、`openclaw-gateway.service`（飞书网关，配置见 `docs/openclaw-setup.md`）。13 个 `data-*` timer 分两类（10 业务同步 + 3 基建）及完整时间表见 `docs/ops-runbook.md` §1；定时时刻按 OS 本地 CST。
