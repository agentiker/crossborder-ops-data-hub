...
- **利润卡含今天窗口（2026-07-15 修订）**：
  - **历史天**读 `fact_profit_daily` 预聚合日表（次日凌晨定稿）。
  - **今天**不读日表中的盘中旧快照，而是实时调用 `compute_daily_profit` 按当前订单 / 马帮成本 / 费用数据重算，再与历史天合并。避免开发回填或盘中聚合把“1 单利润”写进日表后污染看板。
  - 所以 `7 天窗口含今天` = **历史 6 天定稿 + 今天当日累计估算**，不是最终结算利润；明天凌晨 `aggregate_profit` 跑完后，今天会变成日表定稿口径。
- **当天扣点延迟估算（2026-07-15 修订）**：TikTok 官方未结算预估费当天覆盖≈0（见 §7.1）。当 `metric_date == business_today()` 且官方费用覆盖订单数低于护栏时，利润卡的**扣点**不用这几条不完整官方行，而用**历史已结算非广告扣点率 × 当日利润口径 GMV**估算；前端扣点 tooltip 显示“历史已结算扣点率估算”、基准窗口、官方覆盖比例。次日官方费用同步/日表聚合后切回官方费用。
- **覆盖天数护栏**：利润卡读预聚合表时，若历史天 `aggregate_profit` 漏跑会静默少算。`get_profit_card` 返回 `coverage_complete`；前端 false 时显提示。含今天窗口中，今天由实时重算计入覆盖天数，不要求 `fact_profit_daily` 已有今日行。历史缺天的根治靠 `aggregate_profit` timer 常态跑 + 回填（`uv run python -m flows.aggregate_profit --days 30`）。
...
- **未付款取消**（原「发货前流失」，2026-07-04 改名）= CANCELLED 且 `paid_time IS NULL`（下单没付 / COD 未确认）——**不是退款**，单列展示避免混淆。含 COD 拒收（货已发出、到货拒付，其实在发货后）+ 下单未付两类；因 COD 单付款前 `paid_time` 恒空，无法从数据区分取消发生在发货前后，故用中性「未付款取消」而非时点具体的「发货前流失」。
...
- **prod 实测锚点（2026-07-03，SasaQueen.id 近 90 天）**：付款后取消 **86 单** / 退款额 **Rp 10,935,465** / 退款率 **0.41%**（分母展示 GMV 26.35 亿）；取消构成 2061 单 = 付款后 86 + 未付款 1975（拆分自洽），其中 COD 1284。退款率 0.41% 属健康低值。
...
- 后端下发顶层 `window{start,end,includes_today,as_of_label}`；前端经营概览标题显"数据截至 MM-DD HH:MM·今日为当日累计"徽章。利润卡含今天时走 §2.2 的混合口径：历史天读日表，今天实时重算；前端提示"今日为当日累计、次日凌晨定稿"，扣点若走历史费率估算则在 tooltip 里明示。
...
**⚠️ 预估费给值时效：预估费率「今天只查得到昨天、当天几乎为空」（2026-07-15 prod 实测）**

TikTok 官方预估费**不在下单当天给**——订单要往后走到"已送达待结算"等状态才生成预估值（接口只返回未结算订单，`unsettled_reason` 多为 `ORDER_DELIVERED_AND_AWAITING_SETTLEMENT`）。prod 真实店按下单业务日实测覆盖率（当天下单数 vs 已有官方预估值的订单数）：

| 下单日相对今天 | 覆盖率 | 实测（2026-07-15 快照） |
|------|------|------|
| 当天（T） | **≈0%** | 07-15：1/256 |
| 次日（T-1） | **≈90%** | 07-14：651/698=93% |
| T-2 及更早 | 稳定 **85~93%** | 07-13~06-28 均 80~93% |

- **永远到不了 100%**：剩余 ~10% 是取消 / 未付款 / 未送达的单，本就拿不到预估、不会补齐（≠数据缺失）。
- **实务口径**：✅「今天的预估费率」实际代表**截至昨天**（~90% 覆盖，够用）；❌ 当天数据近 0，要等次日。
- **⚠️ 评估窗口/趋势剔除当天(T)（2026-07-15 上线）**：当天覆盖≈0%，若纳入，**单笔高费率订单会把窗口/趋势末点顶高**（实测 07-15 仅 1 单、趋势尖到 27%）。故 `get_fee_rate_monitor`（看板）与 `_scan_unsettled_fee_rate`（告警）的评估窗口都**结束于昨天(T-1)**：`eval_end = today − 1`、`eval_start = eval_end − realtime_eval_days + 1`；趋势逐日点也画到 T-1 为止。告警去重键 `last_window_end` 随之变为 T-1（每业务日仍最多一次）。
- **同步节奏**：`data-sync-unsettled-fees.timer` 每天 **01:13 CST**（=WIB 00:13，业务日初）抓一次未结算快照；每早看板最新完整日=前一天。故看板"当前预估费率（近 `realtime_eval_days` 日窗口）"里今天贡献≈0，数字主要由前两日撑，明天同步后今日才补上。
- **两个正交概念别混**：本节讲**覆盖率**（多少订单已有预估值，随天数爬升）；§6.2 / 趋势图"结算中·偏高"讲**数值精度**（预估金额随临近结算下修、偏保守）。近窗"覆盖不全 + 数值偏高"双因素叠加 → 看近窗必须对照**历史已结算基准**、别只看绝对值。
...
| 2026-07-15 | 利润卡含今天窗口规则落档：历史天读 `fact_profit_daily` 日表，今天实时重算后合并；当天官方扣点覆盖不足时用历史已结算非广告扣点率估算，并在前端扣点 tooltip 标注来源/覆盖率/基准窗口。7 天含今天 = 历史定稿 + 今日累计估算，次日凌晨定稿后切回日表 | 本文 §2.2/§4.2、`services/profit_{aggregation,summary}.py`、`web/routes/data.py`、`frontend/src/pages/BoardPage.tsx` |
