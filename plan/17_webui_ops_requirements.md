...
| 6 | 新品销量曲线 + 爆单提醒(单日50) | ✅ 看板新品卡按天曲线(sparkline+展开)+爆单实时告警均已上线 | 按天曲线 + 爆单实时告警(复用 scan+去重缓存) |
...
> | 2 扣点告警+爆单 | ✅ 告警+新品曲线均上线 | 2-A 扣点率告警 + 2-B 爆单提醒 已接 `scan_fulfillment_alerts` 第3/4规则；H 新品按天销量曲线(echarts 前端)已完成(`1f754b7`,看板新品卡 sparkline+展开曲线)；**剩 2-A+ 及时费率告警见 plan/18** |
...
- ✅ #6 新品按天销量曲线（H，已完成 commit `1f754b7`）：看板「近 N 天新品」卡已实现——行内迷你 sparkline（`NewProductSparkline`，爆单品红色+峰值打点）+ 点击展开完整曲线（`NewProductChart`，带轴+爆单阈值虚线+峰值+tooltip），数据 `get_new_product_trends` 的 `series[]`。前端 echarts 已落地，非未做。
