...
class FeishuOAuthConfig(BaseModel):
    ...
    # authorize 请求的 scope：表示"用户需增量授予的权限"。默认空=不传，让已授权用户静默发码、
    # 不再每次弹同意页（contact:user.id:readonly 已在应用后台开通、且授权累积无需重申）。
    # 万一某权限需用户显式增量授予，再经 env FEISHU_OAUTH__OAUTH_SCOPE 加回。
    oauth_scope: str = ""
