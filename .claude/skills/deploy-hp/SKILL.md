...
- **前端 SPA**：`deploy.sh` **自带前端构建**（line 70-86：加载 nvm → `npm install && npm run build` → 产出 `frontend/dist`）。SPA 由 data-hub 从 `frontend/dist` 经 StaticFiles 挂在 `/app`，`dist` 不入库（gitignore）。改了 `frontend/` 跑 `deploy.sh --pull` 即生效，**无需手动 build**（仅当 deploy.sh 报 `⚠️ 未探测到 npm` 才手动补，见 §2.5）。node v22 在 nvm 路径，非交互 shell 默认 PATH 无 npm——deploy.sh 已处理加载，不用自己 export。
...
`deploy.sh` 幂等做：`git pull` → `uv sync` → **构建前端 SPA（`npm install && npm run build`）** → `init_db`（只建不存在的表）→ 装 systemd units → `daemon-reload` + enable 所有 timer + `data-hub.service`。
...
| `frontend/` SPA（页面/组件/`api.ts`） | （**deploy.sh 自动构建前端**，无需手动 build；**不**重启 web） | dist 不入库、由 StaticFiles 直接服务，deploy.sh build 完即时生效 |

典型代码修复（本次 P0 在 `core/db.py`）：`--pull --restart-web`。
全栈改动（后端 + 前端，如 plan17 渠道饼图）：`--pull --restart-web`（deploy.sh 顺带 build 前端，无需额外步骤）。

### 2.5 前端构建（deploy.sh 自动；手动仅 fallback）

`deploy.sh` **自带前端构建**（line 70-86：加载 nvm → `npm install && npm run build` → 产出 `frontend/dist`，StaticFiles 直接读盘服务、**无需重启 web**）。所以改了 `frontend/` 跑 `deploy.sh --pull` 即可，**不必再手动 build**。

仅当 deploy.sh 打印 `⚠️ 未探测到 npm，跳过前端构建` 时（node 不在 nvm 标准路径）才手动补：
...
坑：`frontend/package-lock.json` 偶有本机 build 残差挡 `git pull`，pull 前 `git checkout -- frontend/package-lock.json` 丢弃即可（远端 lock 权威，install 会重建）。deploy.sh 用 `npm install`（非 `ci`）能自调和多数差异。
