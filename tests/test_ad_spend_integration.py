...
def _txn(create_dt, gmv_max, tap, affiliate):
    ...
    def iter_statement_transactions(self, statement_id, **kwargs):
        ...
        if statement_id == "S1":
            yield {"currency": "IDR", "transactions": [
                _txn(datetime(2026, 6, 8, 10, 0), "100.00", "20.00", "5.00"),
            ]}
        else:  # S2
            yield {"currency": "IDR", "transactions": [
                _txn(datetime(2026, 6, 8, 11, 0), "10.00", "2.00", "1.00"),
                # 跨日：UTC 6/8 17:30 → 印尼 6/9
                _txn(datetime(2026, 6, 8, 17, 30), "50.00", "0.00", "0.00"),
            ]}
...
def test_flow_end_to_end_writes_and_reads_back(session, monkeypatch):
    ...
    count = flow_mod.save_ad_spend_to_db.fn(
        pages, rows, statement_time_ge=ge, statement_time_lt=lt,
        country="ID", shop_id="shop-1",
    )
    # 两个业务日分组：6/8 与 6/9
    assert count == 2
...
def test_flow_save_is_idempotent(session, monkeypatch):
    ...
    for _ in range(2):
        ...
        flow_mod.save_ad_spend_to_db.fn(
            pages, rows, statement_time_ge=1000, statement_time_lt=2000,
            country="ID", shop_id="shop-1",
        )
