...
**切换默认范围（印尼 / 全部 / 拉美）**：
- **不调用任何 Data API**（不要查 overview/inventory/orders/...，**也不要调 /scopes**）。
...
- 注意 `scope_key` 是英文 slug，**不要让用户输入** slug；用上表里的中文短语（"印尼"等）作为用户该发的内容。

**通用约束**：
- 本期（08a）**没有跨消息的持久 binding**——下一条消息如果不带范围词或上面的菜单短语，回到默认全量。这是已知限制，08b 期会用 webhook + binding 表持久化。
