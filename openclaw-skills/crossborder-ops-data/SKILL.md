...
`===ONBOARDING_BEGIN===` 与 `===ONBOARDING_END===` 这两行**只是边界标记，不要发给用户**。直接从 `👋 你好` 开始，到 `再看一次指引发「指引」。` 结束（共 11 行含 2 个空行）。
...
- ❌ 自己重新组织成 `我能帮你做什么 / 使用方式 / 我能做的 vs 不能做的` 三段结构（你只需要复制上面 11 行）
...
正确的回答 = 第一个字符是 `👋`、最后一行是 `切换范围发「印尼」或「全部」，再看一次指引发「指引」。`、中间逐字等于上面 ONBOARDING 块。11 行（含 2 个空行）。
