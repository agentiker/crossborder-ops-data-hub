...
def _row_hash(prev_hash: Optional[str], parts: list[str]) -> str:
    canonical = "|".join([prev_hash or ""] + [p or "" for p in parts])
...
def record_audit_event(
    ...
    *,
    ...
    parts = [
        acc or "", event_type, event_action, actor_open_id or "",
        target or "", summary or "", created_at.isoformat(),
    ]
...
def audit_event_canonical_parts(rec: AuditLog) -> list[str]:
    return [
        ...
        rec.target or "", rec.summary or "", rec.created_at.isoformat() if rec.created_at else "",
    ]
