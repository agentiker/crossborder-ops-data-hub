...
def get_ad_spend_summary(
    *,
    ...
    会误导（见 docs/business-rules §6）。三项里：
    - **付费投放**（`paid_ad_spend` = GMV Max + TAP）：预算型/撬动型投放，ROAS 只该对它算。
    - **达人佣金**（`affiliate_commission` = 联盟 CPS）：成交后按比例分佣，跟着 GMV 走、无撬动，
      拿它当广告投入算 ROAS = 佣金率倒数，无意义。
    ...
    **覆盖护栏**（`complete` / `settled_through`）：广告费来自**已结算** statement，且
    ...
    try:
        ...
        return {
            ...
            "paid_ad_spend": gmv_max_f + tap_f,       # 付费投放 = GMV Max + TAP（ROAS 用此口径）
            "gmv_max_fee": gmv_max_f,
            "tap_commission": tap_f,
            "affiliate_commission": _to_float(affiliate),  # 达人佣金（CPS），与投放分开
            "currency": currency,
...
def get_roas(
    *,
    ...
    """ROAS = GMV ÷ **付费投放**（paid_ad_spend = GMV Max + TAP）。付费投放为 0 → roas=None（不臆造）。

    ⚠️ 分母用**付费投放**而非营销总支出（2026-06-28 口径修正，见 get_ad_spend_summary / docs §6）：
    达人 CPS 佣金（affiliate_commission）成交才付、跟着 GMV 走，算进 ROAS 分母会把"佣金率倒数"
    误当广告效率。达人主导的店付费投放可能为 0 → roas=None，诚实留空。
    ...
    return {
        ...
        "paid_ad_spend": paid_ad_spend,               # 付费投放（ROAS 分母）
        "roas": roas,
