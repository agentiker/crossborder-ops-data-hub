...
async def _collect_weekly(open_id: str, period) -> dict:
    ...
    # 2) 动销率
    sell = get_sell_through(start_date=cur_sd, end_date=cur_ed, cutoff=cutoff, by_create=True, **_sc)
    # 3) 新品表现
    new_prods = get_new_product_performance(
        start_date=cur_sd, end_date=cur_ed, cutoff=cutoff, limit=10, by_create=True, **_sc,
    )
