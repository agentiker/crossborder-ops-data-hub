...
- GET /login：签一个短时效 state（防 CSRF）→ 302 跳飞书授权页。
- GET /callback：校 state → 用 code 换 token → 取 open_id → 签登录 cookie → 302 回 /board。
- GET /logout：清 cookie → 回 /board（随即触发重新登录）。

登录成功后固定回 /board（不接受外部 next，避免开放重定向）。token 只用一次取 open_id 即丢弃。
"""
...
async def login():
    """发起登录：生成签名 state → 跳飞书授权页。"""
    nonce = secrets.token_urlsafe(16)
    state = _make_signed(nonce, _STATE_TTL)
    try:
...
async def callback(
    ...
    """飞书回调：校 state → 换 token → 取 open_id → 签 cookie → 回看板。"""
    if not state or _verify_signed(state) is None:
